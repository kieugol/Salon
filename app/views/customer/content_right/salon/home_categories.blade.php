                                ...
                                <?php
                                         ...
                                         ?>  
                                            <tr>
                                                ...
                                                <td><img src="<?php echo Asset('/').'store/upload/images/'.$result->thumb_products ?>" />
                                                    <?php if($result->hits_products==1){ ?>
                                                        <div class="img-hot"><img src="{{Asset('store/images/hosts.png')}}" width="25"></div>
                                                    <?php } ?>
                                                    <?php if($result->is_new_products==1){ ?>
                                                        <div class="img-new"><img src="{{Asset('store/images/new.png')}}" width="25"></div>
                                                    <?php } ?>
                                                    <?php if($result->is_sale_products>0){ ?>
                                                        <div class="img-sale"><img src="{{Asset('store/images/sale.png')}}" width="25"></div>
                                                     <?php } ?>
                                                </td>
                                                <td>{{$result->code_products}}</td>
                                                <td><?php echo $result->name_products ?></td>
                                                <td>
                                                    {{$result->name_products_categories}}
                                                </td>
                                                <td>
                                                <?php 
                                                    foreach ($list_productsMf as  $value)
                                                    if( $result->manufacture_id == $value->idProductsManufacture )
                                                        echo $value->name_products_manufacture;
                                                ?>
                                                </td>
                                                <td>{{$result->quanlity_products}}</td>
                                                <td><?php echo number_format($result->price_products,0,",",".") ?></td>
                                                <td><a href="<?php echo Asset('/').'administrator/products/enable?id='.$result->idProducts.'&stt='.$result->enable_products ?>" class="btn btn-flat <?php if($result->enable_products==1) echo "btn-primary" ; else echo "btn-danger"; ?>  btn-sm">
                                                    <?php if($result->enable_products==1) echo "enable"; else echo "disable"; ?>
                                                    </a>
                                                </td>
