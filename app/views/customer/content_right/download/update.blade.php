...
<aside class="right-side">
   <form id="productForm" method="post" action="<?php echo Asset('/') ?>administrator/download/update"  accept-charset="uft8" role="form">
    <section class="content-header">
                    <h1>Download<small>/ update</small></h1>
                    <div style="text-align: right">
                        <button class="btn btn-primary btn-flat"><span class="glyphicon glyphicon-floppy-save"></span> Save </button>
                        <a href="<?php echo Asset('/') ?>administrator/download/home" class="btn btn-danger btn-flat"><i class="fa fa-fw fa-mail-reply"></i>Back</a>
                    </div> 
                                            ...
                                            <!-- form start -->                
                                                ...
                                                <table class="table " >
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <input type="text" class="form-control" value="{{$list_download['title_download']}}"  name="txt_title" id="txt_title" placeholder="Enter title">
                                                                <input type="hidden" id="txt_id" name="txt_id" value="<?php echo $list_download['idDownload'] ?>" >
                                                            </td>                                                
                                                        </tr>
                                                        <tr>
                                                            <td><label>Categories</label></td>
                                                            <td>
                                                                <select class="form-control" name="sl_cat">
                                                                    <option value="0">Selectcategories</option>
                                                                @foreach($list_downloadcat as $result)
                                                                    <option <?php if($result->idDownloadCategories== $list_download['Categories_id'] ) echo 'selected'; ?> value="{{$result->idDownloadCategories}}">{{$result->name_download_categories}}</option>
                                                                @endforeach
                                                                </select>  
                                                            </td>
                                                        </tr>
                                                         <tr>
                                                            ...
                                                            <td><input type="text" value="{{$list_download['url_download']}}" class="form-control"  name="txt_url" id="txt_url" placeholder="Enter url"></td>                                                
                                                        </tr>

                                                        <tr>
                                                            <td><label >Sort</label></td>
                                                            <td><input type="text" class="form-control" value="{{$list_download['ordering_download']}}"  name="txt_sort" id="txt_sort" placeholder="Enter Sort"></td>                                                
                                                        </tr>
                                                        <tr>
                                                            <td> <label >Status</label></td>
                                                            <td>
                                                                <input type="radio" name="rd_status" <?php if($list_download['enable_download'] == 1) echo 'checked="checked"'?>   value="1">&nbsp;On &nbsp;&nbsp;     
                                                                <input type="radio" name="rd_status" <?php if($list_download['enable_download'] == 0) echo 'checked="checked"'?>  value="0"> Off
                                                            </td>
                                                        </tr>
