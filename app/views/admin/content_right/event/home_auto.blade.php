...
@section('content')
 <aside class="right-side">
    ...
    <section class="content-header">
        <h1>Sự kiện đăng ký chờ duyệt<small>/ home</small></h1>
        <div style="text-align: right">
            <a href="<?php echo Asset('/') ?>administrator/event/home-auto-register" class="btn btn-default  btn-flat"><span class="glyphicon glyphicon-refresh"></span></a>
                        ...
                        <table id="tb_home" class="table table-bordered table-striped">
                            ...
                            @foreach ($list_event as $result)
                                <tr>
                                    ...
                                    <td>
                                         <img src="<?php echo Asset('/') . 'store/upload/images/' . $result->thumb_event ?>" />
                    @if($result->price_sale_event > 0)
                     <div class="discounticonblk-phantram">
                        <span class="giovangtxtsp-phantram">Sale</span>
                        <span class="giovangdiscounttxtsp-phantram">{{$result->percent_event}}%</span>
                    </div>
                    @endif
				    </td>
									...
									<td>
                                    <a href="<?php echo Asset('/') . 'administrator/event/enable?id=' . $result->id_event . '&stt=' . $result->enable_event ?>" class="btn btn-flat <?php if ($result->enable_event == 1) {
	echo "btn-primary";
} else {
	echo "btn-danger";
}
?>  btn-sm">
                                        <?php if ($result->enable_event == 1) {
	echo "Đã duyệt";
} else {
	echo "Chờ duyệt";
}
