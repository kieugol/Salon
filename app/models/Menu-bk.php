...
class Menu extends \Eloquent {
	...
	protected $fillable = array('title_menus','alias_menus','default_menus','ordering_menus','des_menus','enable_menus'
		,'meta_title_menus','meta_desc_menus','meta_key_menus','idArticlesCategories','created_at','updated_at');
	/* --------------------Admin---------------------------*/ 
	...
	public function getArticles($alias){
		$articles = $this
		->where('alias_menus','=',$alias)
		->first();
		return $articles; 
		}
	// get list id articles have in menu
	public function getArrayIdArticles(){
		$listMenu = $this->get();
		$data = array();
		$i=0;
		foreach ($listMenu as $key => $value) {
			if($listMenu[$key]->idArticlesCategories!=0)
			{
				$data[$i]= $listMenu[$key]->idArticlesCategories;
				$i++;
			}
		}
		if(count($data)>0){
	...
	public function getMenuFooterOther($alias){
		...
		->where('enable_menus','=',1)
		->where('alias_menus','!=',$alias)
		->where('alias_menus','!=','trang-chu')
		->where('alias_menus','!=','home')
		->get();
		return $listMenu;
	}
