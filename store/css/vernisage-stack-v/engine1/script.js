...
function ws_book(p, n, b) {
    ...
    var i = f("ul", b);
    b = b.parent();
    var k = f("<div>").addClass("ws_effect").appendTo(b),
        e = p.duration,
        d = p.perspective || 0.4,
        g = p.shadow || 0.35,
        a = p.noCanvas || false,
    ...
    this.go = function(r, q) {
        ...
        m.trigger("effectStart", k);
        var E = (q == 0 && r != q + 1) || (r == q - 1),
            v = n.get(r),
            G = n.get(q);
        ...
        if (l) {
            ...
            var C = f("<div>").css(y).css({
                left: "50%",
                background: "url(" + (E ? G : v).src + ") right",
                backgroundSize: "auto 100%",
                width: "50%",
                perspective: perspect
            }).appendTo(s);
            var I = f("<div>").css(y).css({
                background: "url(" + (E ? G : v).src + ")",
                backgroundSize: "auto 100%",
            ...
            var F = f("<div>").css(y).css({
                background: "url(" + (E ? v : G).src + ") right",
                backgroundSize: "auto 100%",
            ...
            var u = f("<div>").css(y).css({
                zIndex: 1,
                opacity: 1
            }).appendTo((E ? D : C)).clone().appendTo((E ? F : I)).css("opacity", g).clone().appendTo((E ? C : D)).css("opacity", 1).hide().clone().appendTo((E ? I : F)).css("opacity", g).hide()
        } else {
            if (a) {
                var x = f("<div>").css({
                    position: "absolute",
        ...
        j = new h(E, z, D, C, I, F, B, H, A, x, function() {
            m.trigger("effectEnd", r);
            s.remove();
            j = 0
        });
        return r
    ...
    function h(A, s, C, B, y, x, v, w, u, z, E) {
        if (l) {
            if (!A) {
                s *= -1;
                var D = B;
            ...
            y.css("transform", "rotateY(" + s + "deg)").children().fadeIn(e / 2);
            C.children().fadeOut(e / 2, function() {
                x.show().css("transform", "rotateY(0deg)").children().fadeOut(e / 2);
                B.children().fadeIn(e / 2)
            })
        } else {
            if (a) {
                v.show();
